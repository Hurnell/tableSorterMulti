(function ($) {
    ...
    var sorted = 0;
    var totalColumns = 0;
    $.fn.tableSorterMulti = function (columns) {
        var table = this;
        totalColumns = columns;
        this.on('click', 'tr.sorter-header th', function () {
            sorted++;
            $(this).addClass('sorting');
            $(this).hasBeenSorted() ? sorted-- : sorted > totalColumns ? null : $(this).addClass('sort-' + sorted);
            if (!$(this).hasClass('no-sort') && sorted <= totalColumns) {


                var tableHeaders = $(this).parent().children();
                var wasAscending = !$(this).hasClass('descending');
                tableHeaders.each(function () {
                    if (!$(this).hasBeenSorted() || $(this).hasClass('sorting')) {
                        $(this).hasClass('ascending') ? $(this).removeClass('ascending') : null;
                        $(this).hasClass('descending') ? $(this).removeClass('descending') : null;
                    }
                    $(this).hasClass('sorting') ? $(this).removeClass('sorting') : null;
                });

                wasAscending ? $(this).addClass('descending') : $(this).addClass('ascending');
                var rows = $('tbody  tr', table).not('.sorter-header, .other-header').get();
                sortTable(table, rows, tableHeaders);
            } else {
                resetSortedColumns();
            }
        });
    ...
    function getSortType(element) {
        var sortType = 'string';
        switch (true) {
            case element.hasClass('is-number'):
                sortType = 'numeric';
                break;
            case element.hasClass('is-date'):
                sortType = 'date';
                break;
            case element.hasClass('case-sensitive'):
                sortType = 'caseSensitive';
                break;
        }
        return sortType;
    }
    $.fn.hasBeenSorted = function () {
        var isSorted = false;
        for (var i = 1; i <= totalColumns; i++) {
            if (this.hasClass('sort-' + i)) {
                isSorted = true;
            }
        }
        return isSorted;
    };
    function resetSortedColumns() {
        $('tr.sorter-header th').each(function () {
            for (var i = 1; i <= totalColumns; i++) {
                $(this).hasClass('sort-' + i) ? $(this).removeClass('sort-' + i) : null;
                $(this).hasClass('ascending') ? $(this).removeClass('ascending') : null;
                $(this).hasClass('descending') ? $(this).removeClass('descending') : null;
            }
        });
    ...
    function sortTable(table, rows, tableHeaders) {

        rows.sort(function (a, b) {
            return calculateSort(a, b, tableHeaders, 1);
        });
    ...
    function calculateSort(a, b, tableHeaders, index) {
        var header = $('tr.sorter-header th.sort-' + index);
        var column = tableHeaders.index(header);
        var direction = header.hasClass('ascending') ? -1 : 1;
        var sortType = getSortType(header);
        var A = getVal(a, column, sortType);
        var B = getVal(b, column, sortType);
        var ret = 0;
        switch (true) {
            case A < B:
                ret = 0 - direction;
            ...
            case A > B:
                ret = 0 + direction;
                break;
            case A === B && ++index <= totalColumns:
                ret = calculateSort(a, b, tableHeaders, index);
                break;
        }
        return ret;
    }
    ...
    function test() {
        $('tr.sorter-header th[class*=sort-]').each(function () {
            $(this).html('found');
        });
    }
}(jQuery));
